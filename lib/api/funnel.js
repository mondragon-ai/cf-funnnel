...
exports.app.post('/handleSubmit', async (req, res) => {
    ...
    if (snapShot.exists()) {
        ...
        // Update Customer 
        const customer = await index_2.stripe.customers.update(data.STRIPE_UUID, {
            email: data.email,
            name: data.name,
            shipping: {
                name: data.name,
...
exports.app.post('/charge', async (req, res) => {
    ...
    try {
        ...
        // Send Order
        await node_fetch_1.default("http://127.0.0.1:8080/sendOrder", {
            method: 'post',
...
exports.app.post('/sendOrder', async (req, res) => {
    ...
    if (snapShot.exists()) {
        ...
        const shopify_order = await node_fetch_1.default(URL + `draft_orders.json`, {
            ...
            .then(json => json);
        // Complete Order
        await node_fetch_1.default(`https://{{STORE_NAME}}.myshopify.com/admin/api/2022-07/draft_orders/${shopify_order.id}/complete.json`, {
            method: 'post',
            body: JSON.stringify(draft_order_data),
            headers: HEADERS_ADMIN
        })
