 ...
 * Test API Route
 */
exports.app.post('/create-sub', async (req, res) => {
    res.status(200).json({ msg: "SUCCESS" });
});
/**
 * Create Scene
...
const initialCharge = (FB_UUID, product, bump) => {
    setTimeout(() => {
        ...
        node_fetch_1.default("http://127.0.0.1:8080/charge", {
            method: 'post',
            body: JSON.stringify({
                FB_UUID: FB_UUID,
                product: product,
...
const sendOrder = (FB_UUID) => {
    ...
    setTimeout(() => {
        ...
        const f = FB_UUID;
        // initiate Order 
        node_fetch_1.default("http://127.0.0.1:8080/sendOrder", {
            method: 'post',
...
const completeOrder = (draftID) => {
    ...
    ;
    // Complete Order
    const shopify_order = node_fetch_1.default(URL + `draft_orders/${draftID}/complete.json`, {
        method: 'put',
        headers: HEADERS_ADMIN
    })
...
exports.app.post('/addProduct', async (req, res) => {
    ...
    try {
        ...
        await node_fetch_1.default("http://127.0.0.1:5001/shopify-recharge-352914/us-central1/api/customers/charge", {
            method: 'post',
            body: JSON.stringify({
                FB_UUID: FB_UUID,
                product: product,
